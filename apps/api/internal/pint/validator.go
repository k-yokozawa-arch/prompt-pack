...
func (v Validator) Validate(draft InvoiceDraft) ValidationResult {
	...
	issue, errIssue := parseDate(draft.IssueDate)
	due, errDue := parseDate(draft.DueDate)
	if errIssue != nil {
		errors = append(errors, errItem("JP-PINT-REQ-003", "issueDate", errIssue.Error()))
	}
	if errDue != nil {
		errors = append(errors, errItem("JP-PINT-REQ-004", "dueDate", errDue.Error()))
	}
	if errIssue == nil && errDue == nil && due.Before(issue) {
	...
	for i, line := range draft.Lines {
		...
		if !contains(v.Config.ValidUnitCodes, line.UnitCode) {
			errors = append(errors, errItem("JP-PINT-CODE-001", path+".unitCode", "Invalid unit code"))
		}
		if !contains(v.Config.ValidTaxCategory, line.TaxCategory) {
